    ...
    <section>
      ...
      <div>
        <ul v-bind:key="value.id" v-for="(key, value) in mensajes">
            ...
            email: {{key.email}} <br>
            Image: <br><img v-bind:src="key.image" alt="message-photo" width="150" style="margin-top: 5px; margin-bottom:5px"/>   
        </ul>
      </div>
    </section>
  </body>    
...
export default {
    ...
    methods: {
        async enviarMensaje(){
            ...
            await this.axios.post(api, data , {headers: {"Authorization" : `Bearer ${token}`}})
